...
const Navbar = ({ classes }) => {
  ...
  return (
    ...
    >
      ...
      <ul className="list-none">
        <li
          className={`${scrollTopper <=260 ? "text-foreground" : "text-mutedForeground"} cursor-pointer`}
          onClick={() => scrollToSection(0,2000)}
        >
          Home
        </li>
        <li
          className={`${scrollTopper>=260 && scrollTopper <=1370 ? "text-foreground" : "text-mutedForeground"} cursor-pointer`}
          onClick={() => scrollToSection(670,2000)}
        >
          Skills
        </li>
        <li
          className={`${scrollTopper >= 1370 && scrollTopper <=2170 ? "text-foreground" : "text-mutedForeground"} cursor-pointer`}
          onClick={() => scrollToSection(1600,2000)}
        >
          Projects
        </li>
        <li
          className={`${scrollTopper >= 2170 ? "text-foreground" : "text-mutedForeground"} cursor-pointer`}
