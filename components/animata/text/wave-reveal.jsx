...
const Word = ({ isWordMode, word, index, offset, delay, duration, className }) => {
  if (isWordMode) {
    return word;
  }
    ...
    <span
      ...
      style={
        isWordMode
          ? {
              animationDuration: `${duration}`,
              animationDelay: createDelay({ index, offset, delay }),
            }
          : undefined
      }
    >
...
export default function WaveReveal({
  ...
  const { nodes } = useMemo(
    () =>
      words.reduce(
        (acc, word, index) =>
          createAnimatedNodes(acc, word, index),
        {
          nodes: [],
          offset: 0,
    ...
    [text, direction, mode, duration, delay, blur, letterClassName]
  );
